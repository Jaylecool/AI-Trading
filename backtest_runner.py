...
class BacktestRunner:
    ...
    def __init__(self, symbol: str = 'AAPL', data_filepath: str = None):
        """
        Initialize backtest runner
        
        Args:
            symbol: Stock symbol to backtest
            data_filepath: Path to historical data CSV (auto-detected if None)
        """
        self.symbol = symbol
        self.data_filepath = data_filepath or f'data/{symbol}_stock_data_with_indicators.csv'
        self.data = None
    ...
    def load_data(self) -> None:
        ...
        engine = BacktestingEngine(symbol=self.symbol)
...
def main():
    ...
    for symbol in symbols:
        data_filepath = f'data/{symbol}_stock_data_with_indicators.csv'
        if not os.path.exists(data_filepath):
            print(f"\n[SKIP] No data file for {symbol}: {data_filepath}")
            continue
        ...
        # Initialize runner
        runner = BacktestRunner(symbol=symbol)
    ...
    print(f"Results saved to backtest_results_<SYMBOL>.json and strategy_comparison_<SYMBOL>.csv")
